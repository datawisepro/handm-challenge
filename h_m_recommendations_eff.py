...
class HMModelEff(nn.Module):
  def __init__(self, article_shape, eff_name='b0'):
    super(HMModelEff, self).__init__()
    ...
    self.input = nn.Conv2d(18, 32, kernel_size=1)
...
model_eff = HMModelEff((len(le_article.classes_), 512))
...
model = train(model_eff, train_loader, epochs=50)
...
test_df["prediction"] = inference(model_eff, test_loader)
